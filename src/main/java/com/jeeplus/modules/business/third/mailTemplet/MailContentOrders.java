 ...
 * @author afanti
 */
public class MailContentOrders extends MailContentHTMLTempletAbstract {

	private List<String> tableHeader ;		//表头
	private List<List<String>> tableItems ; //表内容条目
	private String plainText ; 					//纯文本说明
	private List<String> orderChartImages ; //订单统计图表
	
	/**
	 * @param text 纯文本说明
	 * @param tableHeader 表头 使用ArrayList , 无表格数据,使用null
	 * @param tableItems  表内容条目 使用ArrayList , 无表格数据,使用null
	 * @param orderChartImages 订单统计图表
	 */
	public MailContentOrders(String text , List<String> tableHeader , List<List<String>> tableItems , List<String> orderChartImages) {
		this.plainText = text ;
		this.tableHeader = tableHeader ;
		this.tableItems = tableItems ;
		this.orderChartImages = orderChartImages ;
	}
	...
	protected String content() {
		...
		if (orderChartImages != null && orderChartImages.size() > 0) {
			sBuilder.append("<div style='padding:10px 20px;'>统计图表如下：</div>");
			sBuilder.append("<div style='padding:10px 20px;'>");
			for (String img : orderChartImages) {
				sBuilder.append("<img src='" + img + "' width='800' height='600'>");
			}
			sBuilder.append("</div>");
		}
