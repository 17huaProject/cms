...
appendCity($(".area-duoxuan"), -1);

function appendCity(thiscon, selectNum) {
	dataInput = thiscon;
    selectTagsNum = selectNum;
    if (selectTagsNum >= 0 ) {
        $('.data-result').find('strong').text(selectTagsNum);
    } else {
        $('.data-result').html('<em>可选择多项</em>');
    }
    if ($(dataInput).val() != "") {
        var inputarry = $(dataInput).val().split(',');
        var inputarryname = $(dataInput).val().split(',');
...
function selectProvince(type, con, isremove) {
    ...
    if (type == "all") {
        ...
        strVarCity += '  <p class="data-title">热门词</p>';
        ...
        strVarCity += '      </ul>';
        strVarCity += '    </div>';
        strVarCity += '    <p class="data-title">全部分类</p>';
    ...
    //显示下一级
        ...
        if (typeof (relations[$(con).data("code")]) != "undefined") {
            ...
            if (selectTagsNum != 1) {
                ...
                if (selectTagsNum >= 0 && $('.data-result span').length >= selectTagsNum) {
                    $('.data-error').slideDown();
                    setTimeout("$('.data-error').text('最多只能选择 "+selectTagsNum+" 项').hide()", 1000);
                    return false;
                } else {
...
function submit() {
    ...
    //$(".area-duoxuan",window.opener.document).val(name);
    opener.fillTags(name) ;
    window.close();
}
