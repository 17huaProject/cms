	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#userType").change(function(){
				...
				$("#artist").empty();
				if(userType == "5"){  //画师
					$.post("${ctx}/wms/artist/listAllArtists",
						function(data){
							console.log(data);
					   		if(data.code == 0){
					   			var artists = data.data ;
					   			$("#artist").append("<option selected=\"selected\" value=\"\">请选择画师</option>");
						   		for(var i=0; i<artists.length ;i++){
						   			$("#artist").append("<option value=\""+artists[i].id+"\">" +artists[i].realname + " - " + artists[i].phone + "</option>");
						   		}
					   		}
							if(data.code == 1108){
					   			console.log("查询出错");
					...
					$("#artist-label").html("<font color='red'>*</font>关联画师:");
					$("#artist").addClass("required");
				}else{
					$("#artist").append("<option value=\"\">请选择画师</option>");
					$("#artist-label").html("关联画师:");
					$("#artist").removeClass("required");
				}
			});
	...
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
		      ...
		      <tr>
		         <td class="active"><label class="pull-right"><font color="red">*</font>用户类型:</label></td>
		         <td>
		         ...
		         <td class="active"><label id="artist-label" class="pull-right">关联画师:</label></td>
		         <td>
		         	<select id="artist" name="artistId" class="form-control">
		         		<option value="${artist.id}">${artist.realname}</option>
		         	</select>
		         	<label class="error">仅当用户类型为画师时, 此项必选</label>
		         </td>
		      </tr>
